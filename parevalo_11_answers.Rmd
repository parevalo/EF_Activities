---
title: "Exercise 11 - Model Assessment"
...
LAI assimilation seems to have a big effect on GPP, which is not surprising since its value is directly linked to LAI values. It also has a big effect on NPP, which is directly related to GPP and on autotrophic respiration (Ra), which is dependend on NPP values. The effect seems to be smaller (but still noticeable) for litter and much smaller for CWD, especially for the first 100 time steps, which makes sense since its relationship with LAI is less direct. 
...
According to the observed vs predicted plots the performance of both models seems very similar, with both capturing, with the regression line of model vs observed being very close to the 1 to 1 relationship. The particle filter shows better statistics overall but the margin is small, with RMSE and bias being lower than the ensemble model, and correlation higher. For both models, there's a high variance around the -10 value, indicating that the model may need to be refined to better account for these negative fluxes. 
...
The average flux seems to perform pretty well for this case, with an observed vs predicted plot which regression line is closes to the 1 to 1 line. The statistics also seem to be in favor of the "climatology" model, with the lowest values for RMSE and bias and the highest correlation. However, the problem is that a simple average probably cannot capture extreme or unexpected events (among other things), whereas we would expect the process model to better account for these.
...
It is interesting to see the members of the ensemble plotted in the Taylor diagram, as they show how the large variability obtained for any given single run can be overcome by running multiple models to get a better sense of the output. The three models still seem to be very close to each other, with "climatology" still looking slightly better than the others.
...
Compared to the figure in the cited article the simple models made a remarkably good job, being very close to the performance of most of the best models in that paper, which probably have a lot more computational burden and are more complex to parametrize!
...
Noon seems to be the time of day with the highest uncertainty for the three models considered. Climatology model seems to be the closest to the actual observation, while both PF and ensemble models are slighlt off in the high and low values. 
...
MODIFY PLOTS - MISSING PLOTS!

Longer time scales seem to be dominating the data, with semester and montly scales showing the highest intensity. However, it is also intersting to notice the red spots around the day scale but only for the warmer months (e.g. summer time). For the residuals, the scales are roughly similar, but there is a higher semester intensity throughout the year.
...
Temperature is good to explain model error but only for the most extreme temperature values, while PAR better capture these fluctuations across most of its range, but mostly in the middle section. Error seems to be higher for the PAR values that are less than 1829 and temperatures lower than 27.65 ???
...
The model doesn't seem to capture the functional response to temperature alone, and it only overlaps the data in the first section, for the temperatures below 0. It underpredicts NEP in the mid temperature section and overpredicts it in the higher temperatures, 
